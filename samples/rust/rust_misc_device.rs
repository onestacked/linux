...
impl MiscDevice for RustMiscDevice {
    ...
    fn read_iter(mut kiocb: Kiocb<'_, Self::Ptr>, iov: &mut IovIterDest<'_>) -> Result<usize> {
        ...
        let unique = me.unique.lock();
        // Read the buffer contents, taking the file position into account.
        let read = iov.simple_read_from_buffer(kiocb.ki_pos_mut(), &unique.buffer)?;
    ...
    fn write_iter(mut kiocb: Kiocb<'_, Self::Ptr>, iov: &mut IovIterSource<'_>) -> Result<usize> {
        ...
        let mut unique = me.unique.lock();

        // Replace buffer contents.
        unique.buffer.clear();
        let len = iov.copy_from_iter_vec(&mut unique.buffer, GFP_KERNEL)?;
